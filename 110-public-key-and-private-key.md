...
關於數位簽章\(digital signature\):

* 其使用情境如下:
  若A要發送訊息給B, 那麼B要怎麼確認這個訊息真的是由A所發送的呢?

* 解法:
  A在發送訊息前, 用自己的private key將訊息加密, 之後再傳送給B, 此時B再利用A的public key對收到的訊息解密.
  根據對稱式金鑰加密, 我們知道: 若訊息可以正確解密, 就可以確定訊息是由A所發出的; 儘管這個訊息在傳送的過程中被C給攔截了, 且C用A的public key將這段訊息還原成原本的訊息, 但C還是沒有辦法偽裝成A來發送訊息, 因為C沒有A的public key. 因此, 數位簽章的主要作用就是可以確保不可否定性\(non-repudiation\), 其背後的意義表示說授權機制可以很容易地被建立起來.
